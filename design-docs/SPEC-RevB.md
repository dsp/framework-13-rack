...
**REV D REVERTED (2026-06-15, supersedes the Rev D summary above and every "Rev D" parameter value below):** Rev D widened the bay to clear the cooler, which changed the outer envelope — nothing fit the bays already printed, so it was discarded. The build is the pre-Rev-D baseline (W_int 114, pitch 128, carrier plate 121.5, ear blades 26.3, front body 430, faceplate 482.6) plus a recenter that fixes both Rev D flaws inside unchanged outer dimensions: the cooler overhangs the fan-side edge 7.0 (caliper-measured, not the raster-read 9.0), so every board-locating feature shifted +6.84 in carrier-c, centering the 111.83 envelope in the 114 interior (~1.09/side); the USB-C apertures sit at c 32.95 / 73.75, center h19.6 (~2 above PCB top, user-confirmed — not Rev D's h18.4). The same pass chamfered every vent's print-top to a 12 lintel (45° corbel). `scripts/rebuild_all.py` is the authoritative geometry; where this document disagrees with the script, the script wins.
